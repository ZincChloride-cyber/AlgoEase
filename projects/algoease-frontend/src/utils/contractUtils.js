...
class ContractUtils {
  ...
  // Check if contract allows new bounty creation
  async canCreateBounty() {
    try {
      ...
      const status = state[GLOBAL_STATE_KEYS.STATUS];
      if (status === BOUNTY_STATUS.CLAIMED || status === BOUNTY_STATUS.REFUNDED) {
        return { canCreate: true, reason: null };
      }
      
      return { 
        canCreate: false, 
        reason: `There is an active bounty with status: ${this.getStatusName(status)}. Please wait for it to be completed or refunded.` 
      };
    } catch (error) {
      console.error('Failed to check contract state:', error);
      // If we can't check, allow the attempt but warn the user
      return { canCreate: true, reason: 'Could not verify contract state' };
    }
  }
  ...
  async createBounty(sender, amount, deadline, taskDescription, verifierAddress) {
    try {
      ...
      const canCreate = await this.canCreateBounty();
      ...
      // Validate deadline is in the future (with some buffer)
      const now = Math.floor(Date.now() / 1000);
      const bufferSeconds = 60; // 1 minute buffer
      if (deadlineTimestamp <= (now + bufferSeconds)) {
        throw new Error(`Deadline must be at least ${bufferSeconds} seconds in the future. Current time: ${now}, Deadline: ${deadlineTimestamp}`);
      }
      
      // Convert amount to microALGO (ensure consistent calculation)
      ...
      // Get suggested params (use same for both transactions)
      const suggestedParams = await this.getSuggestedParams();

      // Create payment transaction to send funds to contract (must be first)
      // Use the exact same amount in microALGO to ensure consistency
      const paymentTxn = algosdk.makePaymentTxnWithSuggestedParamsFromObject({
        from: sender,
        to: contractAddress,
        amount: amountMicroAlgo, // Use the exact same value
        suggestedParams,
        note: new Uint8Array(new TextEncoder().encode('AlgoEase: Bounty Payment'))
      });

      // Create the application call transaction (must be second)
      // IMPORTANT: accounts array must contain verifierAddress
      const appCallTxn = await this.createAppCallTransaction(
        ...
        [verifierAddress],  // This makes Txn.accounts[0] = verifierAddress
        'AlgoEase: Create Bounty'
      );

      // Verify transaction structure
      console.log('Transaction structure:');
      console.log('  - Payment amount:', paymentTxn.amount);
      console.log('  - Payment receiver (encoded):', algosdk.encodeAddress(paymentTxn.to.publicKey));
      console.log('  - App call args count:', appCallTxn.appArgs.length);
      console.log('  - App call accounts count:', appCallTxn.accounts ? appCallTxn.accounts.length : 0);
      console.log('  - Verifier address (passed):', verifierAddress);
      
      // Assign group ID to transactions
      algosdk.assignGroupID([paymentTxn, appCallTxn]);
      
      console.log('✅ Transaction group created successfully');
