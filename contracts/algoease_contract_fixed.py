"""
AlgoEase Smart Contract - Bounty Escrow Platform (FIXED VERSION)

This contract manages bounties with the following features:
- Create bounty: Client deposits funds to escrow
- Accept bounty: Freelancer accepts the bounty (no payment)
- Approve bounty: Verifier approves completed work, transfers to freelancer
...
FIXED: All box operations now maintain consistent sizes.
"""
...
# ============================================================================
# Box name: "bounty_" + Itob(bounty_id)
# Box data (packed):
...
def status_to_byte(status: Expr) -> Expr:
...
def update_box_status(box_name: Expr, box_data: ScratchVar, new_status: Expr) -> Expr:
    """Update box with new status while preserving exact size"""
    return App.box_put(
        box_name,
        Concat(
    ...
    return Seq([
        ...
        App.box_put(
            ...
            Concat(
                ...
                status_to_byte(STATUS_OPEN),     # status (1 byte)
...
def accept_bounty():
    ...
    return Seq([
        ...
        # Update box with new freelancer and status - preserve exact size
        App.box_put(
            box_name.load(),
            Concat(
                Extract(box_data.load(), CLIENT_OFFSET, Int(32)),
                ...
                Extract(box_data.load(), AMOUNT_OFFSET, Int(8)),
                Extract(box_data.load(), DEADLINE_OFFSET, Int(8)),
                status_to_byte(STATUS_ACCEPTED),  # new status (1 byte)
                Extract(box_data.load(), TASK_DESC_OFFSET, Len(box_data.load()) - TASK_DESC_OFFSET)
            )
        ),
...
def approve_bounty():
    ...
    Args: [method, bounty_id]
    Requires: freelancer address in Txn.accounts[0]
    """
    bounty_id = ScratchVar(TealType.uint64)
    box_name = ScratchVar(TealType.bytes)
    box_data = ScratchVar(TealType.bytes)
    ...
    return Seq([
        ...
        # Require freelancer address in accounts array
        Assert(Txn.accounts.length() >= Int(1)),
        ...
        # Validate freelancer from accounts matches box data
        Assert(Txn.accounts[0] == freelancer.load()),
        ...
        # Transfer funds directly to freelancer using Txn.accounts[0]
        # This ensures the account is available for inner transaction
        InnerTxnBuilder.Begin(),
        InnerTxnBuilder.SetFields({
            TxnField.type_enum: TxnType.Payment,
            TxnField.sender: Global.current_application_address(),
            TxnField.receiver: Txn.accounts[0],  # Use from accounts array
            TxnField.amount: amount.load(),
            TxnField.fee: Int(0),
        }),
        InnerTxnBuilder.Submit(),
        
        # Update box with new status - preserve exact size
        update_box_status(box_name.load(), box_data, STATUS_CLAIMED),
...
def reject_bounty():
    ...
    Args: [method, bounty_id]
    Requires: client address in Txn.accounts[0]
    """
    bounty_id = ScratchVar(TealType.uint64)
    box_name = ScratchVar(TealType.bytes)
    box_data = ScratchVar(TealType.bytes)
    ...
    return Seq([
        ...
        # Require client address in accounts array
        Assert(Txn.accounts.length() >= Int(1)),
        ...
        # Validate client from accounts matches box data
        Assert(Txn.accounts[0] == client.load()),
        ...
        # Send refund to client using Txn.accounts[0]
        # This ensures the account is available for inner transaction
        InnerTxnBuilder.Begin(),
        InnerTxnBuilder.SetFields({
            TxnField.type_enum: TxnType.Payment,
        ...
        # Update box with new status - preserve exact size
        update_box_status(box_name.load(), box_data, STATUS_REJECTED),
...
def refund_bounty():
    ...
    return Seq([
        ...
        # Get client and verifier from box
        client.store(Extract(box_data.load(), CLIENT_OFFSET, Int(32))),
        ...
        # Update box with new status - preserve exact size
        update_box_status(box_name.load(), box_data, STATUS_REFUNDED),
...
def auto_refund():
    ...
    return Seq([
        ...
        # Get amount and client from box
        amount.store(Btoi(Extract(box_data.load(), AMOUNT_OFFSET, Int(8)))),
        client.store(Extract(box_data.load(), CLIENT_OFFSET, Int(32))),
        Assert(amount.load() > Int(0)),
...
if __name__ == "__main__":
    ...
    with open("algoease_approval_fixed.teal", "w") as f:
        f.write(approval_teal)

    with open("algoease_clear_fixed.teal", "w") as f:
        f.write(clear_teal)

    print("[SUCCESS] Smart contracts compiled successfully!")
    print("Files created:")
    print("  - algoease_approval_fixed.teal")
    print("  - algoease_clear_fixed.teal")
