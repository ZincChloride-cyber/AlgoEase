    ...
    try {
      ...
      if (tx.transaction && tx.transaction['confirmed-round']) {
        ...
        if (appId) {
          ...
          // Update creator mnemonic and address (read from contract.env or environment)
          // Note: Never hardcode mnemonics in scripts - use environment variables
          const contractEnvPath = path.join(__dirname, '../contract.env');
          let mnemonic = process.env.REACT_APP_CREATOR_MNEMONIC || '';
          
          if (!mnemonic && fs.existsSync(contractEnvPath)) {
            const contractEnvContent = fs.readFileSync(contractEnvPath, 'utf8');
            const mnemonicMatch = contractEnvContent.match(/REACT_APP_CREATOR_MNEMONIC=(.+)/);
            if (mnemonicMatch) {
          ...
          if (mnemonic) {
            if (envContent.includes('REACT_APP_CREATOR_MNEMONIC=')) {
              envContent = envContent.replace(
                ...
                `REACT_APP_CREATOR_MNEMONIC=${mnemonic}`
              );
            } else {
              envContent += `REACT_APP_CREATOR_MNEMONIC=${mnemonic}\n`;
            }
          }
